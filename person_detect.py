...
class PersonDetect:
    ...
    def predict(self, image):
        ...
        self.input_dict = {self.input_name:self.processed_img}
        out = self.net.infer(self.input_dict)
        detect = out[self.output_name]
    ...
    def draw_outputs(self, coords, image):
        ...
        for box in coords[0][0]:
            if box[2]> self.threshold:
                ...
                ymax = int(box[6] * image.shape[1])
                object_det.append((xmin, ymin, xmax, ymax))
                cv2.rectangle(image, (xmin, ymin), (xmax, ymax), (255, 0, 255), 2)       
        return object_det, image
        #raise NotImplementedError

    def preprocess_outputs(self, outputs):
        return cv2.resize(outputs, (self.output_shape[3], self.output_shape[2]))
        #raise NotImplementedError
